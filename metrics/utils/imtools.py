...
def alignment(src_img, dst_img, max_translation=20, max_angle=2):
    ...
    def get_coords(im, exclude_border=20,num_peaks=50):
        ...
        coords = corner_peaks(corner_tmp[exclude_border:-exclude_border+1,exclude_border:-exclude_border+1],min_distance=10,num_peaks=num_peaks)
    ...
    def match_corner(coord, window_ext=5, max_d = 20):
        ...
        for cr, cc in coords_src:
            if np.abs(cr-r) <= d and np.abs(cc-c) <= d:
                window_warped = src_img[cr - window_ext:cr + window_ext + 1,cc - window_ext:cc + window_ext + 1, :] # img_warped
    ...
    model_robust, inliers = ransac((np.flip(src, axis=-1), np.flip(dst, axis=-1)), EuclideanTransform,
                                   min_samples=2, residual_threshold=5, stop_sample_num = 50 ,max_trials=1000) #residual_threshold=5
